...
class ExifClass {
	...
	enableXmp = function () {
		ExifClass.isXmpEnabled = true;
	}

	disableXmp = function () {
		ExifClass.isXmpEnabled = false;
	}
	...
	getImageData = (img, callback) => {
		...
		if (img.src) {
			if (/^data\:/i.test(img.src)) { // Data URI
				let arrayBuffer = this.base64ToArrayBuffer(img.src);
				...
				let http: null | XMLHttpRequest = new XMLHttpRequest();
	...
	base64ToArrayBuffer = (base64, contentType?: any) => {
	...
	readEXIFData = (file, start) => {
		...
		tags = this.readTags(file, tiffOffset, tiffOffset + firstIFDOffset, ExifClass.TiffTags, bigEnd);

		if (tags.ExifIFDPointer) {
			exifData = this.readTags(file, tiffOffset, tiffOffset + tags.ExifIFDPointer, ExifClass.Tags, bigEnd);
			for (tag in exifData) {
				switch (tag) {
					case "LightSource":
					case "Flash":
					...
					case "FileSource":
						exifData[tag] = ExifClass.StringValues[tag][exifData[tag]];
						break;
					...
					case "ComponentsConfiguration":
						exifData[tag] =
							ExifClass.StringValues.Components[exifData[tag][0]] +
							ExifClass.StringValues.Components[exifData[tag][1]] +
							ExifClass.StringValues.Components[exifData[tag][2]] +
							ExifClass.StringValues.Components[exifData[tag][3]];
						break;
				}
		...
		if (tags.GPSInfoIFDPointer) {
			gpsData = this.readTags(file, tiffOffset, tiffOffset + tags.GPSInfoIFDPointer, ExifClass.GPSTags, bigEnd);
			for (tag in gpsData) {
				switch (tag) {
					case "GPSVersionID":
	...
	readIPTCData = (file, startOffset, sectionLength) => {
		...
		while (segmentStartPos < startOffset + sectionLength) {
			if (dataView.getUint8(segmentStartPos) === 0x1C && dataView.getUint8(segmentStartPos + 1) === 0x02) {
				...
				if (segmentType in ExifClass.IptcFieldMap) {
					dataSize = dataView.getInt16(segmentStartPos + 3);
					segmentSize = dataSize + 5;
					fieldName = ExifClass.IptcFieldMap[segmentType];
	...
	readThumbnailImage(dataView, tiffStart, firstIFDOffset, bigEnd) {
		...
		let thumbTags: any = this.readTags(dataView, tiffStart, tiffStart + IFD1OffsetPointer, ExifClass.IFD1Tags, bigEnd)
...
export const EXIF = new ExifClass();